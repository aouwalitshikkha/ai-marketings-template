...
class GlossaryCategory(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            slug = base
            i = 2
            while GlossaryCategory.objects.filter(slug=slug).exclude(pk=self.pk).exists():
                slug = f"{base}-{i}"
                i += 1
            self.slug = slug
...
class GlossaryTerm(models.Model):
    ...
    category = models.ForeignKey(
        GlossaryCategory, on_delete=models.SET_NULL, null=True, blank=True, related_name='terms')
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['category'],
                         name='glossaryterm_category_idx'),
...
class GlossaryPageConfig(models.Model):
    title = models.CharField(max_length=160, default="AI Marketing Glossary")
...
class GlossaryFAQ(models.Model):
    page = models.ForeignKey(
        GlossaryPageConfig, on_delete=models.CASCADE, related_name="faqs")
